...
from ppx_align.web.state import get_data_root
from ppx_align.core.storage import _load_parquet
...
def get_markdown_resource(filename: str, page_index: int, resource: str) -> FileResponse:
    ...
    return FileResponse(resolved, media_type=_infer_media_type(resolved.suffix))
...
def get_visual_tokens(filename: str, page_index: int) -> list[dict]:
    ...
    for layer_name, parquet_path in layers:
        df = _load_parquet(parquet_path).copy()
        df["layer"] = layer_name
        dfs.append(df)
