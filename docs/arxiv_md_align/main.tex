...
\begin{abstract}
      ...
      specialised tools to traverse the layers between raw information
      ...
      $\LT$ of labelled bounding boxes produced by a layout analyser,
      ...
      semantic similarity.  Second, we realise the model as a
      ...
      controlled OCR-noise injection, and characterise a staircase
...
The obstacle to this programme is that agents must continually
...
the middle sits \emph{information}: a linearised textual
...
a linearised text stream, discarding the spatial structure that a
...
hierarchy of labelled bounding boxes
...
produced by a layout analyser and a Markdown document $\MD$ produced
...
\begin{itemize}
      ...
      \item A \textbf{composable pipeline} (\cref{sec:algo,sec:impl}) ---
            the \texttt{ppx} system --- realising the model through
...
visual tokens and text.  \Cref{sec:formal} formalises the alignment
...
transformer-based recognisers.  The breadth of this progression is
catalogued in recent systematic reviews of the field
...
detector-recogniser pair that supplies line- and word-level
transcriptions.  PaddleOCR\,3.0 \cite{PaddleOCR30} extends this core
with the PP-StructureV3 layout analyser and a Markdown-first
...
understanding and generation.  Both are natural neighbours to our
...
are the closest single-shot analogues to our decomposed pipeline.
The trade-off is familiar: integrated models can be optimised
...
The direct neighbours of our problem are the few lines of prior
...
\cite{huang2022layoutlmv3} is the closest conceptual neighbour we
...
representation-learning mechanism; ours is an output artefact.
...
statement: the alignment artefact itself, not the model that
...
\emph{partial injective function} $\phi : \{0, \ldots, m{-}1\}
      \rightharpoonup \{0, \ldots, n{-}1\}$ that maximises the total
...
$C = -S$, a classical combinatorial-optimisation problem solvable in
polynomial time.
...
realises the partial-function semantics of
...
\emph{after} global optimisation, so it can only prune weak edges; it
...
range we materialise the induced Markdown slice
...
sub-second Jonker--Volgenant solve.  An alternative tokenisation at
...
matching computationally intractable; a character-level tokenisation
...
short AST leaves --- fragmented equations, long itemised lists,
...
\cref{sec:clientserver} and destabilise the cosine score.
...
prohibitive for full pages.  Block-level conditioning localises each
...
document-structure analyser: it produces region- and block-level
bounding boxes labelled with semantic categories (titles,
...
model-load cost is paid once and amortised across every page.
...
subfolder per page containing all artefacts downstream stages need.
The canonical artefacts include the rasterised RGB page image, the
...
complementary layers of validation, one for tabular artefacts and
one for structured values.

Tabular artefacts --- bounding-box tables, word tokens, layout rows
...
\texttt{DocAlignment} serialises directly to the JSON consumed by
the HTTP API (\cref{sec:clientserver}), closing the loop from
algorithmic output to network response with no ad-hoc marshalling
code.
...
alignment --- is a pure function from one validated artefact to
...
new artefact that can be cached, persisted, or fed to multiple
...
to recover the Markdown slice backing any alignment artefact,
...
can produce auxiliary artefacts named \texttt{markdown\_\{level\}.md}
and \texttt{alignment\_\{level\}.json} alongside the canonical
artefacts, one pair per noise level, which \texttt{ppx-bench}
...
The alignment artefacts are only useful if a human --- or an LLM
...
examples expose real OCR artefacts --- collapsed table structure in
...
artefacts the server then reads directly --- while the server runs
...
\begin{figure}[h]
      ...
      \caption{Thumbnails of \texttt{ppx-svelte} viewing the CLIP
            ...
            OCR artefacts --- collapsed tables in the CLIP example,
...
We quantify the pipeline's behaviour along three axes: alignment quality
...
per-label subpopulations.  The per-document composition is summarised
...
intact behaviour to catastrophic failure.
...
generalises: no single document pulls the aggregate in either
direction.
...
\cref{tab:bench-vanilla-bimodality} summarises the groups.
...
\begin{table}[h]
      ...
      \caption{Line-level mean alignment score by block-label group.
            ...
            context is given in-row where it clarifies the behaviour.
...
\cref{fig:bench-noise} visualises the same data as per-noise kernel
...
later propagates into line-level scores.  We hypothesise that the
...
partial-credit behaviour in between.
...
range.  Document-level generalisation is therefore robust, even as
...
this section summarises what was contributed, what the contributions
...
Second, \cref{sec:algo,sec:impl} presented a \textbf{composable
      pipeline} that realises the formulation.  The algorithmic core is
...
PDFs rendered by PaddleOCR / PPStructure\,V3.  Behaviour on
...
\paragraph{Dependence on the upstream layout analyser.}
...
Markdown / scientific text pairs should improve behaviour on
...
layout-tree neighbourhood rather than discarding it.  This would
...
Taken together, these directions share a common structure: none
require re-architecting the pipeline, and all rest on the typed
boundaries that \cref{sec:impl} already exposes.  In that sense the
...
targeted upgrades at the points the benchmark shows matter most.
...
\begin{figure}[p]
      ...
      \caption{Full-size \texttt{ppx-svelte} viewing the CLIP paper.
            ...
            artefacts in the Markdown panel are a known OCR limitation;
