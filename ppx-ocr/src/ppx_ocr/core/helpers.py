...
def draw_bboxes(
    ...
    for row in df_annotation[cols].itertuples():
        ...
        cv2.rectangle(overlay, (x0, y0), (x1, y1), color, thickness=1, lineType=cv2.LINE_AA)
        if label_column:
            label = str(getattr(row, label_column))
            cv2.putText(overlay, label, (x0, y0 - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1, cv2.LINE_AA)
